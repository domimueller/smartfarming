...
class ScriptController extends ABaseController
{
     ...
     * @static
     */
    private static $MapboxScripts = ['ct-mapbox' => ['file' => 'mapbox.min.js', 'dependencies' => ['jquery']]];
    ...
    /**
     * Registers mapbox script (Frontend)
     *
     * @return void
     *
     * @author Markus Langenegger <markuscubetech.ch>
     * @since 1.7.0
     * @version 1.0.0
     */
    public static function registerMapboxScripts()
    {
        foreach (self::$MapboxScripts as $handle => $data) {
            $path = get_template_directory() . self::ASSETS_DIR . $data['file'];
            $uri = get_template_directory_uri() . self::ASSETS_DIR . $data['file'];
            self::registerScript($handle, $path, $uri, $data['dependencies']);
        }
