...
class EventCard extends BaseCard
{
    ...
    public function __construct($postId)
    {
        ...
        $this->description = $this->getField('page_lead');
        $this->start = date_i18n('d. F Y', $this->getField('start_date_timestamp'));
        $this->end = strtotime($this->getField('end_date_timestamp'));
    ...
    private function calculateDaysBeforeStart()
    {
        ...
        $eventDay = date('dmY', $this->getField('start_date_timestamp'));
        if ($now === $eventDay) {
            return _x('Heute', 'Event tag if starting today', 'wptheme-basetheme');
        }
        
        $now = current_time('timestamp');
        $difference = intval($this->getField('start_date_timestamp')) - $now;
